...
func (m *Person) Validate() error {
	...
	if !_Person_Name_Pattern.MatchString(m.GetName()) {
		return PersonValidationError{
			field:  "Name",
			reason: "value does not match regex pattern \"^[^\\\\d\\\\s]+( [^\\\\d\\\\s]+)*$\"",
		}
	}
	...
	if !_Person_Mobile_Pattern.MatchString(m.GetMobile()) {
		return PersonValidationError{
			field:  "Mobile",
			reason: "value does not match regex pattern \"^(0|86|17951)?(13[0-9]|15[012356789]|166|17[3678]|18[0-9]|14[57])[0-9]{8}$\"",
		}
	}
...
var _Person_Name_Pattern = regexp.MustCompile("^[^\\d\\s]+( [^\\d\\s]+)*$")

var _Person_Mobile_Pattern = regexp.MustCompile("^(0|86|17951)?(13[0-9]|15[012356789]|166|17[3678]|18[0-9]|14[57])[0-9]{8}$")
